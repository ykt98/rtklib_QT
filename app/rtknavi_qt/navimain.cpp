...
#endif
//将度数转换为度-分-秒  convert degree to deg-min-sec --------------------------------------------
...
int  MainWindow::ExecCmd(const QString &cmd, int show)
{
    ...
    return QProcess::startDetached(cmd); /* 不支持显示选项 FIXME: show option not yet supported */
}
// 构造器 constructor --------------------------------------------------------------
...
MainWindow::~MainWindow()
{
 ...
 //表单创建时回调 callback on form create --------------------------------------------------
void  MainWindow::showEvent(QShowEvent *event)
{
    ...
    connect(pushButton_2, SIGNAL(clicked()), this, SLOT(pushButton_2Click()));
    ...
    connect(pushButton,SIGNAL(clicked(bool)),this,SLOT(BtnLogStrClick()));
...
void MainWindow::comboBoxChange(int)
{
    TYPE = comboBox->currentIndex();
    ClearTable();
    tableWidget_2->clear();
...
void MainWindow::ClearTable(void)
{
    ...
    //TYPE = comboBox->currentIndex();
    switch (TYPE) {
    case  0: SetRtcm();   break;
    case  1: SetNav();      break;
    case  2: SetGnav();     break;
    case  3: SetNav();      break;
    case  4: SetNav();      break;
    case  5: SetNav();      break;
    case  6: SetSbsNav();   break;
        tableWidget_2->setColumnWidth(0, tableWidget_2->width());
        break;
    }
    tableWidget_2->setVisible(true);
}
...
//        for (k = 0; k < NFREQ ; k++) {
//            //p[i] = obs[i].P[k];
//            /*sprintf(P[i], "%f", obs[i].P[k]);
//            sprintf(D[i], "%f", obs[i].D[k]);
//            sprintf(SNR[i], "%f", obs[i].SNR[k]);*/
//            sprintf(Q[i], "%f", obs[i].SNR[k]);
//        }
//    }
//
//
...
void MainWindow::pushButton_2Click()
{
    savenav(tableWidget_2);
}
...
void  MainWindow::SvrStart(void)
{
    ...
    }else { //单一位置的平均值 average of single position
        PrcOpt.refpos=1;
        for (i=0;i<3;i++) PrcOpt.rb[i]=0.0;
    }
...
// 间隔计时器上的回调 callback on interval timer -----------------------------------------------
...
void  MainWindow::Timer1Timer() 
{
    ...
    switch (TYPE) {
    case  0: break;
    case  1:  ShowNav(SYS_GPS);break;
    case  2: ShowGnav();       break;
    case  3: ShowNav(SYS_GAL); break;
    case  4: ShowNav(SYS_QZS); break;
    case  5: ShowNav(SYS_CMP); break;
    case  6: ShowSbsNav();     break;
    }*/
}
void  MainWindow::Timer2Timer()
{
    if (!isVisible()) return;
    TYPE = comboBox->currentIndex();
    //ShowObs();
...
void MainWindow::SetRtcm(void)
{
    ...
    for (i = 0; i < tableWidget_2->columnCount(); i++) {
        tableWidget_2->setColumnWidth(i, width[i]);//width[i] * Fontscale / 96
    }
    tableWidget_2->setHorizontalHeaderLabels(header);
}
//---------------------------------------------------------------------------
void MainWindow::ShowRtcm(void)
{
    rtcm_t rtcm;
    double pos[3] = { 0 };
    int i = 0, j, format;
    ...
    tableWidget_2->setRowCount(26);
    tableWidget_2->setHorizontalHeaderLabels(header);

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Format")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(format == STRFMT_RTCM2 ? tr("RTCM2") : tr("RTCM3")));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Message Time")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(tstr));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Station ID")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(QString::number(rtcm.staid)));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Station Health")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(QString::number(rtcm.stah)));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Sequence No")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(QString::number(rtcm.seqno)));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Station Pos X/Y/Z (m)")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(QString("%1,%2,%3").arg(rtcm.sta.pos[0], 0, 'f', 3).arg(rtcm.sta.pos[1], 0, 'f', 3).arg(rtcm.sta.pos[2], 0, 'f', 3)));

    if (norm(rtcm.sta.pos, 3) > 0.0) ecef2pos(rtcm.sta.pos, pos);
    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Station Lat/Lon/Height (deg,m)")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(QString("%1,%2,%3").arg(pos[0] * R2D, 0, 'f', 8).arg(pos[1] * R2D, 0, 'f', 8).arg(pos[2], 0, 'f', 3)));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("ITRF Realization Year")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(QString::number(rtcm.sta.itrf)));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Antenna Delta Type")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.sta.deltype ? tr("X/Y/Z") : tr("E/N/U")));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Antenna Delta (m)")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(QString("%1,%2,%3").arg(rtcm.sta.del[0], 0, 'f', 3).arg(rtcm.sta.del[1], 0, 'f', 3).arg(rtcm.sta.del[2], 0, 'f', 3)));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Antenna Height (m)")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(QString::number(rtcm.sta.hgt, 'f', 3)));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Antenna Descriptor")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.sta.antdes));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Antenna Setup Id")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(QString::number(rtcm.sta.antsetup)));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Antenna Serial No")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.sta.antsno));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Receiver Type Descriptor")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.sta.rectype));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Receiver Firmware Version")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.sta.recver));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Receiver Serial No")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.sta.recsno));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("RTCM Special Message")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.msg));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("Last Message")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.msgtype));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("# of RTCM Messages")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(format == STRFMT_RTCM2 ? mstr1 : mstr2));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("MSM Signals for GPS")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.msmtype[0]));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("MSM Signals for GLONASS")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.msmtype[1]));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("MSM Signals for Galileo")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.msmtype[2]));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("MSM Signals for QZSS")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.msmtype[3]));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("MSM Signals for SBAS")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.msmtype[4]));

    tableWidget_2->setItem(i, 0, new QTableWidgetItem(tr("MSM Signals for BeiDou")));
    tableWidget_2->setItem(i++, 1, new QTableWidgetItem(rtcm.msmtype[5]));
}



void MainWindow::FUZHI(void)
{
    obsd_t obs[MAXOBS * 2];
    char tstr[64], id[32], * code;
    int i ,k,n= 0;
    ...
    for (i = 0; i < n; i++) {
        ...
        satno2id(obs[i].sat, id);
        list.append(id);
        list.append(obs[i].rcv);
        for (k = 0; k < NFREQ; k++) {
            list.append(obs[i].P[k]);
        }
        for (k = 0; k < NFREQ ; k++) {
            list.append(obs[i].L[k]);
        }
        for (k = 0; k < NFREQ; k++) {
            list.append(obs[i].D[k]);
        }
        
        for (k = 0; k < NFREQ ; k++) {
            list.append(obs[i].SNR[k] * 0.25);
        } 
        for (k = 0; k < NFREQ; k++) {
            list.append(obs[i].LLI[k]);
        }
        for (k = 0; k < NFREQ; k++) {
            code = code2obs(obs[i].code[k], NULL);
            if (*code) list.append(QString(tr("L%1")).arg(code));
            else       list.append("-"); 
        }
    }
}
...
void  MainWindow::ShowObs(void)
{
    ...
    rtksvrunlock(&rtksvr);
    NN = NN + n;
    tableWidget->setRowCount(n + 1 < 2 ? 0 : NN);
    //Console->setRowCount(1024);
    tableWidget->setColumnCount(3 + (NFREQ + nex) * 6);
    //Label->setText("");
    tableWidget->setHorizontalHeaderLabels(header1);
    //K++;
    for (i = 0; i < n; i++) {
        int j = 0;
        //int K = 0;
        time2str(obs[i].time, tstr, 3);
        tableWidget->setItem(i + icount, j++, new QTableWidgetItem(tstr));
        satno2id(obs[i].sat, id);
        tableWidget->setItem(i + icount, j++, new QTableWidgetItem(id));
        tableWidget->setItem(i + icount, j++, new QTableWidgetItem(QString::number(obs[i].rcv)));
        for (k = 0; k < NFREQ + nex; k++) {
            tableWidget->setItem(i + icount, j++, new QTableWidgetItem(QString::number(obs[i].P[k], 'f', 3)));
        }
        for (k = 0; k < NFREQ + nex; k++) {
            tableWidget->setItem(i + icount, j++, new QTableWidgetItem(QString::number(obs[i].L[k], 'f', 3)));
        }
        for (k = 0; k < NFREQ + nex; k++) {
            tableWidget->setItem(i + icount, j++, new QTableWidgetItem(QString::number(obs[i].D[k], 'f', 3)));
        }
        for (k = 0; k < NFREQ + nex; k++) {
            tableWidget->setItem(i + icount, j++, new QTableWidgetItem(QString::number(obs[i].SNR[k] * 0.25, 'f', 1)));
        }
        for (k = 0; k < NFREQ + nex; k++) {
            tableWidget->setItem(i + icount, j++, new QTableWidgetItem(QString::number(obs[i].LLI[k])));
        }
        for (k = 0; k < NFREQ + nex; k++) {
            code = code2obs(obs[i].code[k], NULL);
            if (*code)  tableWidget->setItem(i + icount, j++, new QTableWidgetItem(QString(tr("L%1")).arg(code)));
            else        tableWidget->setItem(i + icount, j++, new QTableWidgetItem(""));
        }

    }
    icount = icount + n;
}
void  MainWindow::SetObs(void)
{
    ...
    for (i = 0; i < 3; i++) {
        tableWidget->setColumnWidth(j++, width[i] * Fontscale / 96);
        header1 << label[i];
    }
    for (i = 0; i < NFREQ + nex; i++) {
        tableWidget->setColumnWidth(j++, 80 * Fontscale / 96);
        header1 << (i < NFREQ ? QString(tr("P%1 (m)")).arg(freq[i]) : QString(tr("PX%1 (m)")).arg(i - NFREQ + 1));
    }
    for (i = 0; i < NFREQ + nex; i++) {
        tableWidget->setColumnWidth(j++, 85 * Fontscale / 96);
        header1 << (i < NFREQ ? QString(tr("L%1 (cycle)")).arg(freq[i]) : QString(tr("LX%1 (cycle)")).arg(i - NFREQ + 1));
    }
    for (i = 0; i < NFREQ + nex; i++) {
        tableWidget->setColumnWidth(j++, 60 * Fontscale / 96);
        header1 << (i < NFREQ ? QString(tr("D%1 (Hz)")).arg(freq[i]) : QString(tr("DX%1 (Hz)")).arg(i - NFREQ + 1));
    }
    for (i = 0; i < NFREQ + nex; i++) {
        tableWidget->setColumnWidth(j++, 30 * Fontscale / 96);
        header1 << (i < NFREQ ? QString(tr("S%1")).arg(freq[i]) : QString(tr("SX%1")).arg(i - NFREQ + 1));
    }
    for (i = 0; i < NFREQ + nex; i++) {
        tableWidget->setColumnWidth(j++, 15 * Fontscale / 96);
        header1 << "I";
    }
    for (i = 0; i < NFREQ + nex; i++) {
        tableWidget->setColumnWidth(j++, 30 * Fontscale / 96);
        header1 << (i < NFREQ ? QString(tr("C%1")).arg(freq[i]) : QString(tr("CX%1")).arg(i - NFREQ + 1));
    }
...
void MainWindow::savenav(QTableWidget* tableWidget)
{ 
    ...
    QMessageBox::information(NULL, QString::fromLocal8Bit("!"), QString::fromLocal8Bit("start"), QMessageBox::Yes, QMessageBox::Yes);
}


void MainWindow::SaveObs(QTableWidget* tableWidget)
{
    QString filepath = QFileDialog::getSaveFileName(this, tr("Save log"), ".", tr("Microsoft Office 2013 (*.xlsx)"));//获取保存路径
    if (!filepath.isEmpty()) {
       ...
       /* QString B = "A" + QString::number(1) + ":" + "U" + QString::number(1);
        QAxObject* range = worksheet->querySubObject("Range(QString)", B);
        range->setProperty("Value", header1);*/

        for (int i = 0; i < rowCount; i++) {
            QList<QVariant> aline;
            for (int j = 0; j < colCount; j++)
            {
                aline.append(QVariant(list.at(j + colCount * i)));
            }
            QVariant conv(aline);
            rowdata.append(conv);
        }
        QVariant d(rowdata);
        QString A = "A" + QString::number(2) + ":" + "U" + QString::number(rowCount+1);
        QAxObject* range1 = worksheet->querySubObject("Range(QString)", A);
        range1->setProperty("Value", d);
...
void MainWindow::SetNav(void)
{
    ...
    tableWidget_2->setColumnCount(33);
    tableWidget_2->setRowCount(2);

    for (i = 0; i < tableWidget_2->columnCount(); i++) {
        tableWidget_2->setColumnWidth(i, width[i] * Fontscale / 96);
    }
    tableWidget_2->setHorizontalHeaderLabels(header);
}
//---------------------------------------------------------------------------
void MainWindow::ShowNav(int sys)
{
    eph_t eph[MAXSAT];
    gtime_t time;
    QString s;
    ...
    for (k = 0, n = 1; k < MAXSAT; k++) {
        ...
        if (comboBox_2->currentIndex() == 1 && !valid) continue;
        n++;
    }
    if (n < 2) {
        tableWidget_2->setRowCount(0);
    ...
    tableWidget_2->setRowCount(MAXSAT);
    tableWidget_2->setHorizontalHeaderLabels(header);
    ...
    for (k = 0, n = 0; k < MAXSAT; k++) {
        ...
        if (comboBox_2->currentIndex() == 1 && !valid) continue;
        satno2id(k + 1, id);
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(id));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(prn)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(valid ? tr("OK") : tr("-")));
        if (eph[k].iode < 0) s = "-"; else QString::number(eph[k].iode);
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(s));
        if (eph[k].iodc < 0) s = "-"; else QString::number(eph[k].iodc);
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(s));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].sva)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].svh, 16)));
        if (eph[k].toe.time != 0) time2str(eph[k].toe, tstr, 0); else strcpy(tstr, "-");
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(tstr));
        ...
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(tstr));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].A, 'f', 3)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].e, 'f', 8)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].i0 * R2D, 'f', 5)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].OMG0 * R2D, 'f', 5)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].omg * R2D, 'f', 5)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].M0 * R2D, 'f', 5)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].deln * R2D, 'E', 4)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].OMGd * R2D, 'E', 4)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].idot * R2D, 'E', 4)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].f0 * 1E9, 'f', 1)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].f1 * 1E9, 'f', 4)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].f2 * 1E9, 'f', 4)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].tgd[0] * 1E9, 'f', 1)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].tgd[1] * 1E9, 'f', 1)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].tgd[2] * 1E9, 'f', 1)));

        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].cuc, 'E', 4)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].cus, 'E', 4)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].crc, 'E', 4)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].crs, 'E', 4)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].cic, 'E', 4)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].cis, 'E', 4)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].code)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(eph[k].flag)));
        n++;
    }
    tableWidget_2->setRowCount(n);
}
void MainWindow::SetGnav(void)
{
    header.clear();
...
void MainWindow::ShowGnav(void)
{
    ...
    tableWidget_2->setRowCount(NSATGLO);
    tableWidget_2->setHorizontalHeaderLabels(header);
    //int d;
    for (i = 0, n = 0; i < NSATGLO; i++) {
        int j = 0;
        ...
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(s));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].frq)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].svh)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].age)));
        if (geph[i].toe.time != 0) time2str(geph[i].toe, tstr, 0); else strcpy(tstr, "-");
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(tstr));
        if (geph[i].tof.time != 0) time2str(geph[i].tof, tstr, 0); else strcpy(tstr, "-");
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(tstr));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].pos[0], 'f', 2)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].pos[1], 'f', 2)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].pos[2], 'f', 2)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].vel[0], 'f', 5)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].vel[1], 'f', 5)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].vel[2], 'f', 5)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].acc[0], 'f', 7)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].acc[1], 'f', 7)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].acc[2], 'f', 7)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].taun * 1E9, 'f', 1)));
        tableWidget_2->setItem(n, j++, new QTableWidgetItem(QString::number(geph[i].gamn * 1E9, 'f', 4)));
        n++;
    }
    tableWidget_2->setRowCount(n);
}
void MainWindow::SetSbsNav(void)
{
    header.clear();
...
//---------------------------------------------------------------------------
void MainWindow::ShowSbsNav(void)
{
    ...
    for (i = 0, n = 0; i < NSATSBS; i++) {
        ...
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(id));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(prn)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(valid ? tr("OK") : tr("-")));
        ...
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(tstr));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].svh, 16)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].sva)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].pos[0], 'f', 2)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].pos[1], 'f', 2)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].pos[2], 'f', 2)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].vel[0], 'f', 6)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].vel[1], 'f', 6)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].vel[2], 'f', 6)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].acc[0], 'f', 7)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].acc[1], 'f', 7)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].acc[2], 'f', 7)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].af0 * 1E9, 'f', 1)));
        tableWidget_2->setItem(i, j++, new QTableWidgetItem(QString::number(seph[i].af1 * 1E9, 'f', 4)));
        n++;
    }
}
...
//}
// 更新流状态指示器 update stream status indicators ------------------------------------------
...
//    OpenPort=0;
//}
//初始化解决方案缓冲区 initialize solution buffer -----------------------------------------------
...
void  MainWindow::LoadNav(nav_t *nav)
{
    ...
    memset(&eph0,0,sizeof(eph_t));//初始化
...
//从ini文件加载选项 load option from ini file ------------------------------------------------
...
//将选项保存到ini文件  save option to ini file --------------------------------------------------
