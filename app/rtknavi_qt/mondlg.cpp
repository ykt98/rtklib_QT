...
void MonitorDialog::ShowRtk(void)
{
	...
	rtk=rtksvr.rtk;
    thread=(uintptr_t)rtksvr.thread;//xiugai
	cycle=rtksvr.cycle;
	state=rtksvr.state;
...
void MonitorDialog::ShowObs(void)
{
	...
	for (i=0;i<n;i++) {
        ...
        Console->setItem(i+KK,j++, new QTableWidgetItem(tstr));
		satno2id(obs[i].sat,id);
        Console->setItem(i+KK ,j++, new QTableWidgetItem(id));
        Console->setItem(i + KK,j++, new QTableWidgetItem(QString::number(obs[i].rcv)));
		for (k=0;k<NFREQ+nex;k++) {
            Console->setItem(i + KK,j++, new QTableWidgetItem(QString::number(obs[i].P[k],'f',3)));
		}
		for (k=0;k<NFREQ+nex;k++) {
            Console->setItem(i + KK,j++, new QTableWidgetItem(QString::number(obs[i].L[k],'f',3)));
		}
		for (k=0;k<NFREQ+nex;k++) {
            Console->setItem(i + KK,j++, new QTableWidgetItem(QString::number(obs[i].D[k],'f',3)));
		}
		for (k=0;k<NFREQ+nex;k++) {
            Console->setItem(i + KK,j++, new QTableWidgetItem(QString::number(obs[i].SNR[k]*0.25,'f',1)));
		}
		for (k=0;k<NFREQ+nex;k++) {
            Console->setItem(i + KK,j++, new QTableWidgetItem(QString::number(obs[i].LLI[k])));
		}
		for (k=0;k<NFREQ+nex;k++) {
			code=code2obs(obs[i].code[k],NULL);
            if (*code) Console->setItem(i + KK,j++, new QTableWidgetItem(QString(tr("L%1")).arg(code)));
            else       Console->setItem(i + KK,j++, new QTableWidgetItem(""));
		}
