...
void MainForm::BtnExecClick()
{
    ...
    if (InputFile1->currentText()=="") {
        char msg[34] = "error : no rinex obs file (rover)";
        showmsg(msg);
    ...
    if (InputFile2->currentText()==""&&PMODE_DGPS<=PosMode&&PosMode<=PMODE_FIXED) {
        char msg[41] = "error : no rinex obs file (base station)";
        showmsg(msg);

        return;
    }
    if (OutputFile->currentText()=="") {
        char msg[26] = "error : no output file";
        showmsg(msg);
        return;
    }
    if (OutputFile_Text.contains(".obs",Qt::CaseInsensitive)||
        ...
        //char msg[46] = ("error : invalid extension of output file (%s)", qPrintable(OutputFile_Text));
        char msg[50];
        (msg, qPrintable(OutputFile_Text));
        strcat(msg, "error : invalid extension of output file (%s)");
        showmsg(msg);
...
void MainForm::ExecProc(void)
{
    ...
    if (InputFile3_Text!="") {
        ...
        char msg[26] = "error: no navigation data";
        showmsg(msg);
    ...
    // 设置流动站和基站列表 set rover and base station list
...
// 观察文件到导航文件 observation file to nav file ---------------------------------------------
