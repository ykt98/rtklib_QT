...
void  TcpOptDialog::showEvent(QShowEvent* event)
{
    ...
    int index=Path.lastIndexOf(":");//从字符串的末尾向前搜索子字符串，返回所在顺序位置
    QString Str_Text=Path.mid(index);//切割的位置，长度默认-1 存储：之后的地址

    QUrl url("ftp://"+Path.mid(0,index));//：之前的地址

    Addr->insertItem(0, url.host());//添加字符串列表项
    Addr->setCurrentIndex(0);
    Port->setText(QString::number(url.port()));//改变port文本
    ...
    LabelStr->setEnabled(Opt==2);//根据选项不同设置可以显示的选项

    setWindowTitle(ti[Opt]);//题目
