...
static int decoderaw(rtksvr_t *svr, int index)
{
    ...
    for (i=0;i<svr->nb[index];i++) {
        ...
        /*更新rtk服务器 update rtk server */
        if (ret>0) updatesvr(svr,ret,obs,nav,sat,sbsmsg,index,fobs);
        
        /* 收到的观测数据 observation data received */
        if (ret==1) {
...
#endif
{
    ...
    for (cycle=0;svr->state;cycle++) {
        ...
        for (i=0;i<3;i++) {
            p=svr->buff[i]+svr->nb[i]; //输入缓冲器，输入缓冲区中的字节
            q=svr->buff[i]+svr->buffsize;//。。。输入缓冲区大小(字节)
            
            /*从输入流中读取接收器原始/rtcm数据  read receiver raw/rtcm data from input stream */
            if ((n=strread(svr->stream+i,p,q-p))<=0) {
                continue;
            }
            /*将接收器原始/rtcm数据写入日志流 write receiver raw/rtcm data to log stream */
...
*          int     nmeacycle I 非循环请求周期(毫秒)(0:无请求)nmea request cycle (ms) (0:no request)
*          int     nmeareq  I  nmea request type
*                              (0:no,1:base pos,2:single sol,3:reset and single)
* (0:否，1:基本位置，2:单个sol，3:复位和单个)
                       ...
                       solopt_t *solopt, stream_t *moni, char *errmsg)
{
    ...
    if (prcopt->initrst) { /*重启初始化平均位置 init averaging pos by restart */
        svr->nave=0;
        for (i=0;i<3;i++) svr->rb_ave[i]=0.0;
    }
    for (i=0;i<3;i++) { /* 输入/日志流 input/log streams */
        svr->nb[i]=svr->npb[i]=0;
        ...
        for (j=0;j<10;j++) svr->nmsg[i][j]=0; //输入消息计数
        for (j=0;j<MAXOBSBUF;j++) svr->obs[i][j].n=0;//观察数据；观察数据/分配的数量
        ...
        /*设置接收器和rtcm选项 set receiver and rtcm option */
    ...
    /* 设置监控流 set monitor stream */
    svr->moni=moni;
    
    /* open input streams */
    for (i=0;i<8;i++) {
        rw=i<3?STR_MODE_R:STR_MODE_W;//流模式:读取，流模式:写入
    ...
    /* 将start命令写入输入流 write start commands to input streams */
    for (i=0;i<3;i++) {
        if (!cmds[i]) continue;
