...
#define EPHOPT_PREC 1                   /* 星历选项:精确星历 ephemeris option: precise ephemeris */
#define EPHOPT_SBAS 2                   /* ephemeris option: broadcast + SBAS */
#define EPHOPT_SSRAPC 3                 /* ephemeris option: broadcast + SSR_APC */
#define EPHOPT_SSRCOM 4                 /* 星历选项:广播+ SSR_COM ephemeris option: broadcast + SSR_COM */
...
typedef struct {        /* time struct */
    ...
    double sec;         /* 1秒以下的几分之一秒 fraction of second under 1 s */
} gtime_t;
...
typedef struct {        /* navigation data type */
    ...
    double utc_gps[4];  /* 全球定位系统时差-世界协调时参数 GPS delta-UTC parameters {A0,A1,T,W} */
...
typedef struct {        /* processing options type */
    ...
    int sateph;         /* 卫星星历/时钟(EPHOPT _？？？( satellite ephemeris/clock (EPHOPT_???) */
    ...
    int rovpos;         /* 固定模式的流动站位置 rover position for fixed mode */
    int refpos;         /* 相对模式的基本位置 base position for relative mode */
                        /* (0:pos in prcopt,  1:average of single pos, */
                        /*  2:read from file, 3:rinex header, 4:rtcm pos) */
    double eratio[NFREQ]; /* 代码/相位误差比 code/phase error ratio */
